...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/09 11:32:13 by oel-asri          #+#    #+#             */
/*   Updated: 2024/07/09 12:58:45 by oel-asri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol_bonus.h"

static void	color_br(int it, int x, int y, t_fractol *fr)
{
	int	color;
...
void	create_burning_ship(int x, int y, t_fractol *fr)
{
	...
	fr->c.imaginary = map(y + fr->shift_y, IMAGINARY_LOWER_LIMIT, \
	...
	while (it++ < MAX_ITERATION && !point_outside)
	{
		...
		fr->z.imaginary = 2 * fabs(fr->z.reel * fr->z.imaginary) + \
		fr->c.imaginary;
		fr->z.reel = temp;
